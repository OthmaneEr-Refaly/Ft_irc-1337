...
/*   Updated: 2025/09/24 09:36:06 by mobouifr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../Includes/Headers.hpp"

void Channel::executeTopic(Server &server, Client* c, const std::string& topic)
{
	...
	if (!isMember(c))
	{
		...
		c->sendNumericReply(server, ERR_NOTONCHANNEL, _name, "You're not on that channel");
		return;
	}
	...
	if (topic.empty())
	{
		if (!_topic.empty())
		{
			...
			c->sendNumericReply(server, RPL_TOPIC, _name, _topic);
		} 
		else
		{
			std::cout << "Debugging: No topic is set for channel " << _name << std::endl;
			c->sendNumericReply(server, RPL_NOTOPIC, _name, "No topic is set");
		}
		return;
	}
	...
	if (_mode_topic_ops_only && !isOperator(c))
	{
		...
		c->sendNumericReply(server, ERR_CHANOPRIVSNEEDED, _name, "You're not a channel operator");
		return;
	}
	...
	notifyMembers(server, ":" + c->getNick() + "!" + c->getUser() + "@" + c->getHost() +
				" TOPIC " + _name + " :" + topic);
}
...
void handleTopic(Server &server, Client &client, const Command &cmd)
{
	if (cmd.params.empty())
	{
		client.sendNumericReply(server, ERR_NEEDMOREPARAMS, "TOPIC", "Not enough parameters");
		return;
	}
	...
	channel->executeTopic(server, &client, topic);
}
